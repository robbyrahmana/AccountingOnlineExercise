...
class Kelola_soal_mahasiswa_model extends CI_Model
{
   ...
   public function get_siswa_by_kelola_soal_id($kelola_soal_id) {
      ...
      $this->db->select('ksm.id, m.id mahasiswa_id, m.nim, m.nama, m.jumlah_sks, m.ipk, d.nama nama_dosen, m.bukti_pembayaran, ksm.tanggal, ksm.nilai');
   ...
   public function get_soal_essai_by_kelola_mahasiswa_id($id_kelola_soal_mahasiswa, $mahasiswa_id) {
      ...
      $this->db->select('ksj.id, s.soal, s.jawaban, ksj.jawaban jawaban_mahasiswa, ksj.nilai');
      $this->db->from($this->table . ' ksm');
   ...
   public function update()
   {
      ...
      $this->db->where('id', $id);
      return $this->db->update('tbl_kelola_soal_jawaban', $data);
   }
}
